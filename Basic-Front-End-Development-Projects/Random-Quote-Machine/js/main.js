...
twttr.ready(function (twttr){
    $(document).ready(function () {
        getQuote(); // Runs this function

        function getQuote(){ // Define Function
            var quotes = [
            ...
            var quoteSeparate = randomQuote.split("@");
            $(".saying").text(quoteSeparate[0]);
            $(".author").text(quoteSeparate[1]);

            // Assign Twitter Data
            var dataUrl = 'http://test.com';
            // console.log(quoteSeparate[0].length);
            // console.log(dataUrl.length);
            if ((quoteSeparate[0].length + dataUrl.length) > 131 ) {
                var tweetQuote = quoteSeparate[0].slice(0, 131 - (dataUrl.length));
               // console.log(tweetQuote.length);
            }
            else {
                var tweetQuote = quoteSeparate[0];
                // console.log(tweetQuote.length);
            }
            ...
            var tweetBtn = $('<a></a>')
                ...
                .attr('data-text', tweetQuote);
            $('#twitter').append(tweetBtn);
            twttr.widgets.load();

        }

        // when button is pressed pull another quote
